...
description: Specialized agent that generates ToC entries for a single requirement/design document. Processes individual YAML files in specs/.toc_work/.
model: opus
color: cyan
---
...
Processes a single requirement/design document and completes the corresponding entry YAML in `specs/.toc_work/`.
...
|-----------|----------|-------------|
| `entry_file` | Yes | Path to the entry YAML file to process (e.g., `specs/.toc_work/SCR-001.yaml`) |
...
1. Read `{entry_file}`
2. Read the requirement/design document (`specs/{source_file}`) using `_meta.source_file` value
