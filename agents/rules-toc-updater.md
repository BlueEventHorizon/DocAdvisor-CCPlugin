...
description: Specialized agent that generates ToC entries for a single rule document. Processes individual YAML files in rules/.toc_work/.
model: opus
color: orange
---
...
Processes a single rule document (`.md` file under `rules/`) and completes the corresponding entry YAML in `rules/.toc_work/`.
...
|-----------|----------|-------------|
| `entry_file` | Yes | Path to the entry YAML file to process (e.g., `rules/.toc_work/core_architecture_rule.yaml`) |
...
1. Read `{entry_file}`
2. Read the rule document (`rules/{source_file}`) using `_meta.source_file` value
