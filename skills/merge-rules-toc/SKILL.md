...
description: Merges completed YAML entries from rules/.toc_work/ and generates/validates rules_toc.yaml. Used in Phase 3 merge processing of rules_toc updates.
...
Merges completed YAML files from rules/.toc_work/ and generates/validates rules/rules_toc.yaml.
...
```bash
# Full mode (all files)
python3 skills/merge-rules-toc/create_pending_yaml.py --full

# Incremental mode (changed files only)
python3 skills/merge-rules-toc/create_pending_yaml.py
```
...
5. Generate `rules/rules_toc.yaml`

### Incremental Mode

1. Read existing `rules/rules_toc.yaml`
...
4. Write to `rules/rules_toc.yaml`
...
- **Full/Incremental Mode**: YAML files with completed status must exist in `rules/.toc_work/`
- **Delete-only Mode**: `rules/.toc_checksums.yaml` must exist
