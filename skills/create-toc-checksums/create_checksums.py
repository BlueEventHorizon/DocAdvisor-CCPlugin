...
def find_md_files_specs(root_dir, exclude_patterns, target_dirs):
    ...
    for filepath in root_dir.rglob("*.md"):
        ...
        if len(parts) >= 2 and parts[1] in target_dirs:
            md_files.append(filepath)
    return sorted(md_files)
...
def main():
    ...
    root_dir = project_root / config.get('root_dir', f'{target}/').rstrip('/')
    output_file = root_dir / config.get('checksums_file', '.toc_checksums.yaml')
    ...
    if target == 'rules':
        ...
        md_files = find_md_files_specs(root_dir, exclude_patterns, target_dirs)
