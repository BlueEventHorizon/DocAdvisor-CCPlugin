...
Orchestrator command to generate/update `specs/specs_toc.yaml`.
...
3. Select up to 5 files and launch subagents in parallel
    Task(subagent_type: specs-toc-updater, prompt: "entry_file: specs/.toc_work/{ID}.yaml")
    ↓
...
| Condition | Action |
|-----------|--------|
| `--full` + `.toc_work/` exists | Bash: `rm -rf specs/.toc_work` → Start full mode |
...
```bash
test -f specs/.toc_checksums.yaml && echo "EXISTS" || echo "NOT_EXISTS"
```
...
shasum -a 256 specs/main/requirements/APP-001_xxx.md | cut -d' ' -f1
```

### Step 3: Compare Checksums

1. Read `specs/.toc_checksums.yaml`
2. For each file:
...
Task(subagent_type: specs-toc-updater, prompt: "entry_file: specs/.toc_work/SCR-001.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: specs/.toc_work/SCR-002.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: specs/.toc_work/DES-001.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: specs/.toc_work/APP-001.yaml")
Task(subagent_type: specs-toc-updater, prompt: "entry_file: specs/.toc_work/BL-001.yaml")
```
