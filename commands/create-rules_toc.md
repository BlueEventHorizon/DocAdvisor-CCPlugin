...
Orchestrator command to generate/update `rules/rules_toc.yaml`.
...
3. Select up to 5 files and launch subagents in parallel
    Task(subagent_type: rules-toc-updater, prompt: "entry_file: rules/.toc_work/{filename}.yaml")
    ↓
...
| Condition | Action |
|-----------|--------|
| `--full` + `.toc_work/` exists | Bash: `rm -rf rules/.toc_work` → Start full mode |
...
```bash
test -f rules/.toc_checksums.yaml && echo "EXISTS" || echo "NOT_EXISTS"
```
...
1. Read `rules/.toc_checksums.yaml`
2. For each file:
...
Task(subagent_type: rules-toc-updater, prompt: "entry_file: rules/.toc_work/core_architecture_rule.yaml")
Task(subagent_type: rules-toc-updater, prompt: "entry_file: rules/.toc_work/core_coding_rule.yaml")
Task(subagent_type: rules-toc-updater, prompt: "entry_file: rules/.toc_work/layer_ui_rule.yaml")
Task(subagent_type: rules-toc-updater, prompt: "entry_file: rules/.toc_work/workflow_dev_task.yaml")
Task(subagent_type: rules-toc-updater, prompt: "entry_file: rules/.toc_work/format_spec.yaml")
```
